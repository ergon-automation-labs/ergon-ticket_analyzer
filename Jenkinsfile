pipeline {
  ...
  environment {
    BOT_NAME = 'ticket_analyzer_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-ticket_analyzer"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/ticket_analyzer_bot/bin/ticket_analyzer_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyTicketAnalyzer.Release.create()'"
          $RELEASE_BIN eval 'BotArmyTicketAnalyzer.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyTicketAnalyzer.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyTicketAnalyzer.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_ticket_analyzer/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_ticket_analyzer/releases/start_erl.data)
        fi
